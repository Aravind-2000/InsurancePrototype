...
public interface ClientPersonalRepository extends JpaRepository<ClientMaintainPersonal, Long> {
    ...
    @Query(value = "select * from client_maintain_personal where given_name like %:key% and valid_flag = 1 or sur_name like %:key% and valid_flag = 1 or mobile_number like %:key%  and valid_flag = 1 or language like %:key% and valid_flag = 1  or marrital_status like %:key% " +
